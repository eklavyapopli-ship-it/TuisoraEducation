...
const ReviewCards: React.FC<dataDisplay> = ({limit}) => {
    ...
    return (
        ...
        <div className={`max-w-7xl mx-auto h-fit pb-20 mt-20 ${josefin.className} flex flex-col justify-center place-items-center`}>
            ...
            <div className=" mt-20 lg:grid lg:grid-cols-2 gap-x-10 gap-y-10  place-items-center flex flex-col">
                {limitedData.map((item) => (
                    <motion.div
                    whileHover={{scale:1.03}}
                     key={item._id} className={`h-60 p-10 md:w-150 w-fit border-black rounded-2xl hover:shadow-xl flex flex-col relative shadow-md text-black ${josefin.className}`}>
            ...
            <div className="m-20 text-center text-3xl text-blue-600 font-semibold cursor-pointer w-fit group relative"
             onClick={()=>{
                if(showAll==false)
                    {setShowAll(true)}
                else
