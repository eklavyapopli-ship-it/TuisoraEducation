...
const OfferCards = () => {
  return (
    <section className='w-screen h-[80vh]'>
      <div className={`text-center lg:text-6xl text-4xl font-bold text-black ${josefin.className}`}>What We <span className='text-teal-400'>Offer</span></div>
      <div className='w-full h-fit flex justify-center lg:space-y-0 space-y-5  mt-15 lg:space-x-8 lg:mt-20 lg:flex-row flex-col lg:place-content-center place-items-center'>
        {OfferHeads.map((item)=>(
          <div className='flex flex-col lg:h-120 h-fit rounded-2xl border-teal-400 place-items-center border py-8 lg:w-100 w-80  px-15 space-y-5' key={item.id}>
